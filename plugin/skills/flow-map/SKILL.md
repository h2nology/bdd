...
The capture hooks ship with the `bdd-setup` skill's templates for every
language. If the project was set up by `bdd-setup`, capture is already
...
web lane from `${CLAUDE_PLUGIN_ROOT}/skills/bdd-setup/references/<language>.md`
("flow capture"), the mobile lane from
`${CLAUDE_PLUGIN_ROOT}/skills/bdd-setup/references/appium.md`. Do not invent a
...
| Mobile: every screen has the same name | A single-activity Android app with no per-screen identifier. The diagram is worthless until the app exposes one - see `${CLAUDE_PLUGIN_ROOT}/skills/bdd-setup/references/appium.md` section 4 |
