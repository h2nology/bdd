...
|----------|----------|---------|-------------|
| `markdown` | ✓ | — | The markdown body to convert. **`mode=spec` takes structured JSON instead** — see the spec mode section below |
| `mode` | ✓ | — | Layout: `status` \| `incident` \| `plan` \| `explainer` \| `financial` \| `pr` \| `spec` |
...
`mode`, `lang`, and `theme` are orthogonal — every theme renders every mode in every language.
